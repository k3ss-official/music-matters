...
class DownloadService:
    ...
    def download_track(
        ...
        found_url = self._search_youtube(
            search_query, preferred_source or "youtube_music"
        )
        if found_url:
            return self._download_from_url(found_url, output_path, title, artist)

        return DownloadResult(
            success=False, error=f"Could not find track: {search_query}"
        )

    def download(self, url: str) -> Path:
        """Download a track from a URL directly."""
        ...
        try:
            ...
            ydl_opts["outtmpl"] = str(self.downloads_dir / f"{safe_filename}.%(ext)s")
            ...
            temp_files = list(self.downloads_dir.glob(f"{safe_filename}.*"))
            for f in temp_files:
                if f.suffix.lower() != ".wav" and f.exists():
                    self._convert_to_wav(f, output_path)
                    f.unlink()
                    return output_path
                elif f.suffix.lower() == ".wav" and f.exists():
    ...
    def _download_from_url(
        ...
        try:
            ...
            return DownloadResult(
                success=False, title=title, artist=artist, url=url, error=str(e)
            )
