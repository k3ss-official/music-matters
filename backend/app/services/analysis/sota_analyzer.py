...
class SOTAAudioAnalyzer:
    ...
    def analyze(
        ...
        logger.info(
            f"  BPM: {beat_grid.bpm:.1f}, {len(beat_grid.beats)} beats, {len(beat_grid.downbeats)} bars"
        )
        ...
        spectral_curve = self._smooth_curve(
            features["spectral_centroid"], num_points=500
        )
    ...
    def _detect_beat_grid(self, y: np.ndarray, sr: int) -> BeatGrid:
        ...
        try:
            ...
            return BeatGrid(
                bpm=120.0, beats=[], downbeats=[], time_signature=4, confidence=0.0
            )
    ...
    def _get_compatible_keys(self, camelot: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            if same_key:
                compatible.append(
                    {"key": same_key, "camelot": camelot, "type": "perfect"}
                )
            ...
            if next_key:
                compatible.append(
                    {"key": next_key, "camelot": next_camelot, "type": "energy_up"}
                )
            ...
            if prev_key:
                compatible.append(
                    {"key": prev_key, "camelot": prev_camelot, "type": "energy_down"}
                )
            ...
            if rel_key:
                compatible.append(
                    {"key": rel_key, "camelot": rel_camelot, "type": "relative"}
                )
    ...
    def _get_energy_boost_keys(self, camelot: str) -> List[Dict[str, str]]:
        ...
        try:
            ...
            if boost_key:
                energy_keys.append(
                    {"key": boost_key, "camelot": boost_camelot, "type": "energy_boost"}
                )
    ...
    def _extract_features(
        ...
        features["spectral_contrast"] = np.mean(
            librosa.feature.spectral_contrast(y=y, sr=sr), axis=0
        )
    ...
    def _segment_structure(
        ...
        try:
            ...
            if len(beat_grid.downbeats) > 0:
                boundary_times = [
                    self._snap_to_bar(t, beat_grid.downbeats) for t in boundary_times
                ]
            ...
            for i in range(len(boundary_times) - 1):
                ...
                seg_onset = onset_strength[
                    min(start_frame, len(onset_strength) - 1) : min(
                        end_frame, len(onset_strength)
                    )
                ]
                ...
                spectral_mean = (
                    float(np.mean(seg_spectral)) if len(seg_spectral) > 0 else 0
                )
    ...
    def _find_optimal_sample_points(
        ...
        for sp in sample_points:
            ...
            for existing in filtered:
                if (
                    sp.start_time < existing.end_time
                    and sp.end_time > existing.start_time
                ):
                    overlaps = True
                    break
            if not overlaps:
    ...
    def _calculate_energy_score(
        self, start: float, end: float, energy: np.ndarray, duration: float
    ) -> float:
    ...
    def _calculate_beat_alignment_score(
        self, start: float, end: float, beat_grid: BeatGrid
    ) -> float:
    ...
    def _calculate_silence_score(
        self, start: float, end: float, y: np.ndarray, sr: int
    ) -> float:
    ...
    def _calculate_loop_score(
        self, start: float, end: float, y: np.ndarray, sr: int, beat_grid: BeatGrid
    ) -> float:
        ...
        try:
            ...
            for p in phrases:
                ...
                for u in unique_phrases:
                    if (
                        abs(p["start_time"] - u["start_time"]) < 4.0
                        and p["type"] == u["type"]
                    ):
                        is_duplicate = True
                        break
